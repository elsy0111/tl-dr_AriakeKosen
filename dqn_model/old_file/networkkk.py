...
#ボードの初期化
  def _reset(self):
    self.board=state.State(width,height,ike,shiro,first_shokunin,second_shokunin,end_turn=100)
    self.winner=NONE
    self.turn=self.board.now_team
    self.game_end=False
...
#行動による状態変化（石を置く&リバース処理）
  def _step(self, pos):
    ...
    self.game_end=self.board.is_end()
...
#ランダムに石を置く場所を決める（ε-greedy用）
  def random_action(self):
      legal_acrtion=self.board.legal_action()
      result=[]
    ...
    while not env.game_end: # ゲームが終わるまで繰り返す
      if False:# 石が置けない場合はパス
        ...
        while True: # 置ける場所が見つかるまで繰り返す
          ...
          pos = policy_step.action.numpy()[0][0]
          print("アクション:",pos)
          if pos in env.board.legal_action(number=0):
            rewards[env.turn] = 0
            break
          else:
